...
edge1=graph_global.es.find(_from=ournode_global.index,_to=2796)
edge1['txCost']=20

out_edges= np.array( graph_global.es.select(_from=ournode_global.index) )[1:5]

for edge in out_edges:
    
    graph_global=load('graph_with_capacity_midtx.txt',format='pickle')
    
    print 'Node:', edge.target
